...
const LessonPlanner: React.FC<LessonPlannerProps> = ({ 
  ...
  useEffect(() => {
    if (prefill) {
      ...
      setInput(newInput);
      setView('editor');
      if (prefill.autoTrigger === 'both') { handleGenerateBoth(newInput.subject, newInput.grade, newInput.strand, newInput.subStrand); }
      else if (prefill.autoTrigger === 'plan') { handleGeneratePlan(newInput.subject, newInput.grade, newInput.strand, newInput.subStrand); }
      else if (prefill.autoTrigger === 'notes') { handleGenerateNotes(newInput.subject, newInput.grade, newInput.subStrand); }
      onClearPrefill();
    }
  }, [prefill]);

  const handleGenerateBoth = async (subj: string, grd: string, strnd: string, subStrnd: string) => { handleGeneratePlan(subj, grd, strnd, subStrnd); handleGenerateNotes(subj, grd, subStrnd); };
  ...
  const saveCurrentPlan = () => {
    if (!plan) return;
    const newEntry: SavedLessonPlan = { id: Date.now().toString(), dateCreated: new Date().toLocaleDateString(), title: `${(plan.learningArea || '').toUpperCase()} - ${(plan.subStrand || '').toUpperCase()}`, subject: plan.learningArea || '', grade: plan.grade || '', plan: plan };
  ...
  const saveCurrentNotes = () => {
    if (!notes) return;
    const newEntry: SavedLessonNote = { id: Date.now().toString(), dateCreated: new Date().toLocaleDateString(), title: `${input.subject} - ${input.subStrand} Notes`, content: notes, subject: input.subject, grade: input.grade };
    setSavedNotes([newEntry, ...savedNotes]);
    alert("Notes Added to Cloud Archive.");
  };

  const isFormValid = input.subject !== '' && input.subStrand !== '';
  const handleDownloadPlanDocx = () => { if (plan) exportLessonPlanToDocx(plan, userProfile); };
  const handleDownloadNotesDocx = () => { if (notes) exportNotesToDocx(`${input.subject} - ${input.subStrand}`, notes); };
  const handleDeletePlan = (id: string, e: React.MouseEvent) => { e.stopPropagation(); if (confirm("Delete this lesson plan?")) { setSavedPlans(savedPlans.filter(p => p.id !== id)); } };
  const handleDeleteNote = (id: string, e: React.MouseEvent) => { e.stopPropagation(); if (confirm("Delete these study notes?")) { setSavedNotes(savedNotes.filter(n => n.id !== id)); } };
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl min-h-[600px] lesson-plan-card relative print:border-black print:p-0">
                   {loadingPlan ? (
                     <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-10 rounded-[2.5rem] print:hidden">
                        ...
                        <p className="font-black text-indigo-900 uppercase tracking-widest text-[10px]">Architecting Content...</p>
                     </div>
                   ) : plan ? (
                     <div className="animate-in fade-in duration-700">
                        <div className="flex flex-wrap justify-between items-center mb-10 pb-4 border-b print:hidden gap-3">
                           ...
                           <div className="flex gap-2">
                             <button onClick={() => window.print()} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition"><i className="fas fa-print"></i></button>
                             <button onClick={handleDownloadPlanDocx} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition shadow-lg shadow-indigo-100"><i className="fas fa-file-word"></i></button>
                             <button onClick={saveCurrentPlan} className="bg-indigo-500 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition shadow-lg shadow-indigo-100 font-bold"><i className="fas fa-save mr-2"></i> Save</button>
                           </div>
                        </div>
                        
                        <div className="text-center mb-12 border-b-2 border-black pb-4">
                           <h1 className="text-xl font-black uppercase tracking-tight underline underline-offset-4 mb-2 text-black">{(plan.year || 2025)} RATIONALIZED {(plan.learningArea || '').toUpperCase()} LESSON PLANS</h1>
                           <h2 className="text-lg font-black uppercase tracking-widest mb-4 text-black">TERM {(plan.term || 'TWO')} - {(plan.textbook || 'KICD APPROVED TEXT')}</h2>
                        </div>
                        ...
                        <div className="space-y-8 text-[11px] leading-relaxed text-black">
                           ...
                           <div>
                              <h4 className="font-black uppercase tracking-widest mb-2 underline">Learning Outcomes:</h4>
                           ...
                           <div>
                              ...
                              <div className="space-y-6">
                                 <div className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                                    ...
                                    <ul className="list-disc pl-5 space-y-1">{(plan.introduction || []).map((item, idx) => <li key={idx}>{item}</li>)}</ul>
                                 </div>
                                 <div className="space-y-6">
                                    ...
                                    {(plan.lessonDevelopment || []).map((step, idx) => (
                                       <div key={idx} className="ml-2 p-4 border-l-2 border-indigo-100 bg-white shadow-sm rounded-r-2xl">
                                          <p className="font-black mb-1 text-indigo-700 uppercase text-[10px]">Step {idx + 1}: {step?.title || 'Procedure'}</p>
                                          <ul className="list-disc pl-5 space-y-1">{(step?.content || []).map((item, subIdx) => <li key={subIdx}>{item}</li>)}</ul>
                                       </div>
                                    ))}
                                 </div>
                                 <div className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                                    <h5 className="font-black underline mb-2 uppercase text-[10px]">Conclusion (5 minutes)</h5>
                                    <ul className="list-disc pl-5 space-y-1">{(plan.conclusion || []).map((item, idx) => <li key={idx}>{item}</li>)}</ul>
                                 </div>
                              </div>
                           </div>
                        </div>
                     </div>
                   ) : <div className="h-full flex flex-col items-center justify-center text-slate-200 py-32"><i className="fas fa-file-invoice text-5xl mb-4 opacity-10"></i><p className="text-[10px] font-black uppercase tracking-widest">Plan Preview</p></div>}
                </div>
                   ...
                   {loadingNotes ? (
                     ...
                     <div className="animate-in fade-in duration-700">
                        <div className="flex flex-wrap justify-between items-center mb-10 pb-4 border-b print:hidden gap-3">
                           ...
                           <div className="flex gap-2">
                             <button onClick={() => window.print()} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition"><i className="fas fa-print"></i></button>
                             <button onClick={handleDownloadNotesDocx} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition shadow-lg shadow-emerald-100"><i className="fas fa-file-word"></i></button>
                             <button onClick={saveCurrentNotes} className="bg-emerald-600 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition shadow-lg shadow-emerald-200 border border-emerald-500 font-bold"><i className="fas fa-save mr-2"></i> Save</button>
                           </div>
                        </div>
                        <div className="prose prose-sm max-w-none print:prose-black text-black"><ReactMarkdown remarkPlugins={[remarkGfm]}>{notes}</ReactMarkdown></div>
                     </div>
                   ) : <div className="h-full flex flex-col items-center justify-center text-slate-200 py-32"><i className="fas fa-feather-alt text-5xl mb-4 opacity-10"></i><p className="text-[10px] font-black uppercase tracking-widest">Notes Preview</p></div>}
                </div>
             </div>
           )}
        </div>
        ...
        <div className="space-y-8 animate-in fade-in duration-500 print:hidden">
           <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
              <div className="flex bg-slate-100 p-1 rounded-2xl w-fit mb-10 shadow-inner">
                 <button onClick={() => setLibraryTab('plans')} className={`px-10 py-3 rounded-xl font-black text-[10px] tracking-widest transition-all uppercase ${libraryTab === 'plans' ? 'bg-white text-indigo-900 shadow-md' : 'text-slate-500'}`}>Plans ({(savedPlans || []).length})</button>
                 <button onClick={() => setLibraryTab('notes')} className={`px-10 py-3 rounded-xl font-black text-[10px] tracking-widest transition-all uppercase ${libraryTab === 'notes' ? 'bg-white text-indigo-900 shadow-md' : 'text-slate-500'}`}>Notes ({(savedNotes || []).length})</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                 {libraryTab === 'plans' ? (savedPlans || []).map(item => (
                   <div key={item.id} className="group relative p-8 border-2 border-slate-50 rounded-[2rem] hover:border-indigo-200 transition bg-slate-50/50 cursor-pointer" onClick={() => { setPlan(item.plan); setView('editor'); }}>
                      <div className="flex justify-between mb-4"><span className="text-[9px] font-black text-indigo-500 uppercase bg-indigo-50 px-3 py-1 rounded-full">PLAN</span><button onClick={(e) => handleDeletePlan(item.id, e)} className="text-slate-300 hover:text-red-500 transition opacity-0 group-hover:opacity-100"><i className="fas fa-trash-alt text-xs"></i></button></div>
                      <h4 className="font-black text-slate-800 uppercase text-sm mb-1 truncate">{item.title}</h4>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{item.grade} • {item.dateCreated}</p>
                   </div>
                 )) : (savedNotes || []).map(item => (
                   <div key={item.id} className="group relative p-8 border-2 border-slate-50 rounded-[2rem] hover:border-emerald-200 transition bg-slate-50/50 cursor-pointer" onClick={() => { setNotes(item.content); setInput({...input, subject: item.subject, grade: item.grade}); setView('editor'); }}>
                      <div className="flex justify-between mb-4"><span className="text-[9px] font-black text-emerald-500 uppercase bg-emerald-50 px-3 py-1 rounded-full">NOTES</span><button onClick={(e) => handleDeleteNote(item.id, e)} className="text-slate-300 hover:text-red-500 transition opacity-0 group-hover:opacity-100"><i className="fas fa-trash-alt text-xs"></i></button></div>
                      <h4 className="font-black text-slate-800 uppercase text-sm mb-1 truncate">{item.title}</h4>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{item.grade} • {item.dateCreated}</p>
                   </div>
                 ))}
